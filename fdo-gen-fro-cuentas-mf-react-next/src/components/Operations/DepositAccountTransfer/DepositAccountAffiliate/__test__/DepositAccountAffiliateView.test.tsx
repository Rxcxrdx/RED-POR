...
jest.mock("@/context", () => ({
  DepositAccountTransferContext: {
    ...
    Consumer: ({ children }: any) => <div>{children({})}</div>,
  },
}));

// Mockea los componentes compartidos
jest.mock("@/components/SharedComponent", () => ({
  ...
  UserDetailContainer: ({ ContextProvider, hiddenFields }: any) => (
    <div data-testid="mock-user-detail">User Detail Mock</div>
  ),
  AccountFdogenCard: ({ accountData, hiddenFields }: any) => (
...
describe("DepositAccountAffiliateView", () => {
  ...
  const defaultProps = {
    ...
    errorMessage: "",
    pensionAccounts: [],
    filterFormDepositAccountAffiliate: mockFormMethods,
  ...
  const renderComponent = (props = {}) => {
    return render(<DepositAccountAffiliateView {...defaultProps} {...props} />);
  };
  ...
  test("debe renderizar todos los componentes principales", () => {
    renderComponent();
  ...
  test("debe mostrar el loader cuando está cargando", () => {
  ...
  test("debe mostrar mensaje de error cuando existe", () => {
    const errorMessage = "Error de prueba";
    renderComponent({ errorMessage });
    expect(screen.getByText(errorMessage)).toBeInTheDocument();
  });

  test("debe renderizar AccountFdogenCard cuando hay datos de cuenta", () => {
  ...
  test("debe manejar el envío del formulario", () => {
    renderComponent();
    fireEvent.click(screen.getByTestId("submit-button"));
    expect(mockHandleFilterSubmit).toHaveBeenCalled();
  });

  test("debe manejar el reset del formulario", () => {
    renderComponent();
    fireEvent.click(screen.getByTestId("reset-button"));
    expect(mockHandleFilterReset).toHaveBeenCalled();
  });

  test("debe mostrar el modal de búsqueda cuando isNameSearchModalOpen es true", () => {
  ...
  test("debe pasar la configuración correcta al formulario", () => {
    renderComponent();
  ...
  test("no debe mostrar AccountFdogenCard cuando no hay datos", () => {
    renderComponent();
    expect(screen.queryByTestId("mock-fdo-gen-card")).not.toBeInTheDocument();
  });

  test("debe tener el layout correcto", () => {
